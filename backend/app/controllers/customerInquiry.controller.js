...
class CustomerInquiryController {
  ...
  async getUserTickets(req, res) {
    try {
      const userId = req.userId;
      const tickets = await CustomerInquiry.find({ user_id: userId }).sort({ createdAt: -1 });
      return res.status(200).json({
  ...
  async respondToTicket(req, res) {
    try {
      const { id } = req.params;
      const { staff_response, internal_notes, status, priority, resolving_staff_id, send_email } = req.body;
      
      const ticket = await CustomerInquiry.findById(id).populate("user_id", "name email");
      if (!ticket) {
        return res.status(404).json({ message: "Không tìm thấy ticket" });
      }
      ...
      if (send_email && ticket.user_id && ticket.user_id.email && staff_response) {
        try {
          const templatePath = path.join(__dirname, "../../templates/support-response-template.html");
          let html = fs.readFileSync(templatePath, "utf8");
          
          html = html
            .replace("{{customerName}}", (ticket.user_id && ticket.user_id.name) || "Quý khách")
