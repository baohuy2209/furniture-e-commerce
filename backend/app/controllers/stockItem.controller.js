...
class StockItemController {
  ...
  async getStockItemById(req, res) {
    try {
      const stockItem = await StockItem.findById(req.params.id)
        ...
        .populate("warehouse_id");
      if (!stockItem) {
        return res.status(404).json({ message: "Không tìm thấy vật phẩm tồn kho" });
      }
      return res.status(200).json({
  ...
  async createStockItem(req, res) {
    try {
      const { product_variant_id, warehouse_id, quantity_on_hand, quantity_reserved } = req.body;
      
      // Check if already exists
      let stockItem = await StockItem.findOne({ product_variant_id, warehouse_id });
      if (stockItem) {
        return res.status(400).json({ message: "Vật phẩm tồn kho đã tồn tại ở kho này" });
      }
