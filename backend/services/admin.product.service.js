...
class AdminProductService {
  async getAdminProducts({ condition, limit, offset, sortKey, sortDir, stock, minPrice, maxPrice, room }) {
    // Basic filtering
    let products = await Product.find(condition).skip(offset).limit(limit).lean();
    ...
    let formattedProducts = products.map(p => {
      const pVariants = variants.filter(v => v.product.toString() === p._id.toString());
      const pImages = pVariants.flatMap(v => images.filter(img => img.product_variant.toString() === v._id.toString()));
