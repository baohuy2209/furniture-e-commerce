...
export class RequestsWarranty implements OnInit {
  ...
  listWarrantyDetails: (Omit<IWarrantyRequest, 'user_id'> & {
    user_id: IUser;
    images: string[];
  })[] = [];
  ...
  sentRequests: any[] = [
    ...
    },
    {
      ...
      image:
        'https://images.unsplash.com/photo-1538688525198-9b88f6f53126?ixlib=rb-4.0.3&auto=format&fit=crop&w=1674&q=80',
    },
  ];
  ...
  ngOnInit() {
    ...
    this.warrantyService.getAllWarrantyDetails().subscribe({
      next: (details) => {
        this.listWarrantyDetails = details;
        this.updateFilteredRequests();
        this.cdr.detectChanges();
      },
      error: (err) => {
        this.toastService.error('Có lỗi khi tải danh sách bảo hành');
      },
    });
        ...
        forkJoin(detailRequests).subscribe({
          next: (ordersWithItems) => {
            ...
            this.deliveredOrders = (ordersWithItems as IOrderWithItems[]).filter((order) => {
              return order.items.some((i) => i.item.status === 'delivered');
            });
            this.loadingOrders = false;
          },
          error: () => {
  ...
  onOrderChange() {
    ...
    if (order) {
      const deliveredItems = order.items.filter((i) => i.item.status === 'delivered');

      const requests = deliveredItems.map((i) =>
        this.productVariantImageService.getDefaultImageByProductVariantId(
          i.item.product_variant_id,
        ),
      );

      forkJoin(requests).subscribe({
        next: (results) => {
          this.availableProducts = deliveredItems.map((i, index) => ({
  ...
  onProductChange() {
    this.selectedProduct =
      this.availableProducts.find((p) => p.item.product_variant_id === this.selectedProductId) ||
      null;
  }
  ...
  selectProduct(p: SelectedProduct) {
    if (this.selectedProductId === p.item.product_variant_id) {
      this.selectedProductId = '';
      this.selectedOrderItem = '';
      this.onProductChange();
      return;
    }
    this.selectedOrderItem = p.item._id;
    this.selectedProductId = p.item.product_variant_id;
    this.onProductChange();
  }
  ...
  orderItemDelivered(
    ...
    return orderItems.filter((i) => i.item.status === 'delivered');
  }
  updateFilteredRequests() {
    if (this.activeListTab === 'all') {
      this.filteredRequests = this.listWarrantyDetails;
    } else {
      this.filteredRequests = this.listWarrantyDetails.filter(
        (r) => r.warranty_status === this.activeListTab,
      );
    }
  ...
  onFileSelected(event: any) {
    ...
    if (files) {
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        this.selectedFileImage.push(file);
  ...
  submitWarrantyRequest() {
    ...
    const newRequest: Partial<IWarrantyRequest> & {
      productName: string;
      item_image: string;
      images?: string[];
    } = {
